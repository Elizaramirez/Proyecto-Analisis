...
async function Usuario(id) {
    let response = await fetch(`curl --get --include 'https://football-prediction-api.p.rapidapi.com/api/v2/list-markets' \
    -H 'X-RapidAPI-Key: SIGN-UP-FOR-KEY'${id}`)
    let data = await response.json()
    let obj = data.data
...
var letras = document.getElementsByClassName('Area 2')
